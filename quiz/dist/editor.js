...
function showPreview() {
    const typeSel = document.getElementById("editorType");
    const typeval = typeSel.value;
    const quest = document.getElementById("editorQuestion");
    const question = quest.value;
    const preview = document.getElementById("preview");
    let html = `<h4>${question}</h4>`;
    if (typeval === "qcm") {
        const optsInp = document.getElementById("editorQcmOptions");
...
editorForm.addEventListener("submit", (e) => {
    ...
    const questInp = document.getElementById("editorQuestion");
    const question = questInp.value;
    let ansInp = document.getElementById("editorAnswer");
